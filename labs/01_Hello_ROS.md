...
> На схеме показана типичная структура рабочего пространства ROS 2 после выполнения `colcon build`
...
В ROS 2 системное рабочее пространство уже автоматически подключено после установки дистрибутива (обычно через `/opt/ros/jazzy/setup.bash`).  
...
После первой сборки в папке `~/ros2_ws/install/` появится файл `setup.bash`.
